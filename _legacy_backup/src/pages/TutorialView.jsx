...
const TutorialView = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                // Check if it's a Sanity ID (usually alphanumeric and shorter or specific format)
                // For now, we'll try Firebase first, and if not found or specific prefix, use Sanity

                if (tutorialId.startsWith('sanity_') || tutorialId.length < 20) {
                    const sanityId = tutorialId.replace('sanity_', '');
                    const queryStr = `*[_type == "tutorial" && _id == $id][0]{
                        title,
                        chapters[]{
                            title,
                            content,
                    ...
                    const sanityData = await client.fetch(queryStr, { id: sanityId });

                    if (sanityData) {
                        setTutorial(sanityData);
                        setChapters(sanityData.chapters || []);
                        const current = (sanityData.chapters || []).find(c => c.id === chapterId) || sanityData.chapters?.[0];
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    >
                        ...
                        <div className="markdown-content coderafroj-markdown prose-lg prose-invert max-w-none">
                            {isSanity ? (
                                <TutorialRenderer content={currentChapter?.content} />
                            ) : (
                                <ReactMarkdown>
