...
const Notes = () => {
    ...
    const fetchNotes = async () => {
        try {
            ...
            const firebaseSnapshot = await getDocs(q);
            const firebaseNotes = firebaseSnapshot.docs.map(doc => ({
                id: doc.id,
                ...doc.data(),
            ...
            // Fetch Sanity Notes & Tutorials
            const sanityQuery = `*[_type in ["note", "tutorial"]]{
                _id,
                _type,
                title,
                description,
            ...
            const sanityData = await client.fetch(sanityQuery);
            const formattedSanity = sanityData.map(item => ({
                id: `sanity_${item._id}`,
                title: item.title,
                description: item.description,
                tags: item.tags || [],
                category: item.category || (item._type === 'note' ? 'Architecture' : 'Learning'),
                image: item.image,
                createdAt: new Date(item.publishedAt || item._createdAt),
                type: item._type
            }));
            ...
            // Merge and Sort
            const allContent = [...formattedSanity, ...firebaseNotes, ...generatedTutorials].sort((a, b) => b.createdAt - a.createdAt);
