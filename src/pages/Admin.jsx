                ...
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 border-b border-white/5 pb-8">
                    <div>
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <h1 className="text-4xl font-black text-white tracking-tighter">NEXUS CONTROL</h1>
                        </div>
                        <p className="text-dim-text font-mono text-[10px] tracking-[0.2em] uppercase opacity-60">Session ID: {user?.uid?.slice(0, 12)}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    ...
                    <nav className="lg:col-span-2 space-y-3">
                        {[
                            ...
                            { id: 'manage', icon: <Sparkles size={18} />, label: 'REGSITRY' }
                        ].map(tab => (
                            <button
                                key={tab.id}
                            ...
                            >
                                {activeTab === 'manage' ? (
                                    <div className="space-y-16">
                                        ...
                                        {[
                                            { title: 'PROJECT PROTOCOLS', data: projects, col: 'projects', theme: 'primary' },
                                            { title: 'TRANSMISSIONS', data: posts, col: 'posts', theme: 'secondary' },
                                            { title: 'KNOWLEDGE MODULES', data: tutorials, col: 'tutorials', theme: 'accent' }
                                        ].map((sec, i) => (
                                                ...
                                                <div className="grid gap-3">
                                                    {sec.data.map(item => (
                                                        <div key={item.id} className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5 hover:border-white/10 transition-all group">
                                                            ...
                                                            <button onClick={() => deleteItem(sec.col, item.id)} className="p-3 text-red-500/30 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all opacity-0 group-hover:opacity-100">
                                                                <LogOut size={18} />
                                                            </button>
                                                        </div>
                                                    ))}
                                        ...
                                        <header className="flex justify-between items-end">
                                            <div>
                                                <h2 className="text-4xl font-black text-white tracking-tighter uppercase">
                                                    {activeTab === 'project' ? 'Deploy Project' : activeTab === 'blog' ? 'Publish log' : activeTab === 'tutorial' ? 'Create Module' : 'Compose Content'}
                                                </h2>
                                                <p className="text-dim-text text-sm mt-1">Initializing artifact sequence...</p>
                                            </div>
                                            {activeTab === 'chapter' && (
